...
Для воспроизведения решения необходимо сначала запустить sol.py - 
он загрузит и подготовит данные, выделит имя функции и уберет комментарии. 
(Здесь есть небольшое отступление от задания - выделяется не только тело, 
но и прототип с заменой названия функции на  extra_id_i.)
Также скрипт выведет один пример подготовленных данных. Если раскомментировать
два тестовых примера, выведет результаты и для них.

Затем Необходимо запустить sol3.py - он запустит обработку данных моделью
CodeT5+, посчитает Exact Match и ROUGE-1 для обработки тела с комментариями и без.

Вот пример вывода:
```
Metrics for functions with comments:
Exact Match: 0.224
ROUGE-1: 0.5120972222222222
Metrics for functions without comments:
Exact Match: 0.164
ROUGE-1: 0.4038329365079364
```

Также в репозитории есть следующие файлы, сгенерированные скриптами:
- raw.xlsx - изначальный датасет (первые 1000 строк)
- prepared.xlsx - подготовыленный датасет
- done.xlsx - результаты обработки датасета

Основной проблемой была установка зависимостей - пришлось немного поменять requirements.txt.

### Примеры результатов

Данные на 35 строке датасета содержат функцию download. Ей было сопоставлено имя main
в силу схожести прототипов: ```def download(self, **kwargs):```

На 131 строке датасета для функции resetdb было сопоставлено имя initdb - эта функция 
вызывается в конце тела.

Также есть ряд результатов не являющихся валидными именами функций: строки 20, 25, 47, 196 и т. д.
